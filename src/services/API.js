...
const api = axios.create({
    baseURL: process.env.REACT_APP_BACKEND_API_URL || 'http://localhost:3001/api', // Adjust this to your backend URL
    headers: {
...
api.interceptors.request.use(
    (config) => {
        const token = localStorage.getItem('jwtToken'); // Get your stored token
        if (token) {
...
const login = async (username, password) => {
    try {
        ...
        // Assuming your backend returns { token: '...', user: '...' } on successful login
